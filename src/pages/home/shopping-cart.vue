...
<template>
    <div :class="['shopping-cart', { 'shop-card-pb': isKeyBoardHide }]" v-show="isshow">
        ...
        >
            <!--选择框 begin-->
            <div class="select-box" v-if="item.inventory !== 0" @click="danxuan(item)">
                <i
                    class="iconfont iconweixuanzhong"
                    :class="{
                        iconxuanzhong: item.checked
                    }"
                    :for="item.id"
                ></i>
            </div>
            <!-- 库存不存，无法选中 -->
            <div class="select-box unselect" v-if="item.inventory === 0">
                <svg class="icon" aria-hidden="true">
                    <use xlink:href="#iconbukexuan" />
                </svg>
            </div>
            <!--选择框 end-->
                    ...
                    <div class="price-box">
                        <div v-if="item.orderFlag === 0">
                            <div
                                ...
                                :class="[item.obtained && item.inventory === 0 ? 'gray' : '' ]"
                            >
                                {{ item.colorName }} {{ item.sizeName }}
                                <span
                                    class="iconfont iconicondown-copy"
                                ></span>
                            </div>
                            <div
                                v-if="item.inventory > 0"
                                @click="skuDialog(item, index)"
                                class="goods-sku-content"
                                :class="[!item.obtained === true ? 'gray' : '' ]"
                            >
                                {{ item.colorName }} {{ item.sizeName }}
                                <span
                                    class="iconfont iconicondown-copy"
                        ...
                        <span class="goods-stock-tip" v-if="!item.obtained">该商品已下架</span>
                        <span
                            class="goods-stock-tip"
                            v-if="item.obtained && item.inventory === 0"
                        >库存不足</span>
                        <span
                            class="goods-stock-tip"
                            v-if="item.obtained && item.inventory > 0 && item.amount >= item.inventory"
                        >库存剩余{{ item.inventory }}</span>
                    </div>
                </div>
                <!-- 有库存 -->
                <div v-if="item.obtained && item.inventory > 0" class="count-box">
                    <div
                        class="count-btn reduce-btn"
                        @click="reduceCount(item)"
                        :class="{ gray: item.amount === 1 }"
                    >-</div>
                    <!--   v-inputfix -->
                    <input
                    ...
                    <div
                        ...
                        @click="addCount(item)"
                        :class="{ gray: item.amount >= item.inventory }"
                    >+</div>
                </div>
                <!-- 没有库存 -->
                <div v-if="item.obtained && item.inventory === 0" class="count-box">
            ...
            <!-- 无库存（遮罩） -->
            <div v-show="!item.obtained" class="bg-gray"></div>
        </div>
        <!-- 购物车列表 end -->
        ...
        <div v-if="shopCartList.length > 0" class="goods-actions" v-show="isKeyBoardHide">
            <div class="carts-info">
                <div class="van-checkbox" @click="checkedAll()">
                    <i
                        class="iconfont iconweixuanzhong"
                        :class="{ iconxuanzhong: checkAll === true }"
                    ></i>
                ...
                <div class="total">
                    ...
                    <p
                        class="total-reduce"
                        v-if="salesTotal !== 0 && checkedList.length > 0"
                    >优惠减￥{{ salesTotal }}</p>
                </div>
            </div>
            <div class="buy-button" @click="submit">
                <div>
                    <p :class="[false ? '' : 'full']">立即结算({{ rowCartAll }})</p>
                    <!--<p v-if="rowCartAll > 0">省￥{{ rowCartAll }}</p>-->
                </div>
            </div>
        </div>
        <!-- 底部按钮 end -->

        <!-- <footer-bar v-show="isKeyBoardHide"></footer-bar> -->
        ...
        >
            <div class="toast-html" v-show="showDialog == 'unSell'">
                <p
                    v-for="(item,index) in unSellList"
                    :key="item.goodsCode"
            ...
            <div class="toast-html" v-show="showDialog == 'unStock'">
                <p
                    v-for="(item,index) in unStockList"
                    :key="index"
                    :style="{'text-align': 'center'}"
...
export default {
            ...
            if (this.vipInfo.mobilePhone === null || !this.vipInfo.mobilePhone) {
                ...
                Goods.getUseableGood(goodListObj).then((res) => {
                    ...
                    if (res.stockAll !== 1) {   // 商品库存不足
                         ...
                         console.log( this.unStockList,' this.unStockList')
 ...
 // 设置库存校验弹窗显示状态
        ...
        async skuDialog(item, index) {
            ...
            forEachColorArr = goodSkuList.colorList = global.objCombine(temporaryColorArr, 'colorCode')
            ...
            goodSkuList.sizeList = global.objCombine(temporarySizeArr, 'sizeCode')
...
.carts-info {
    ...
    width: 100%;
    height: computed(108);
    box-shadow: computed(0) computed(-2) 0 computed(0) rgba(41, 51, 50, 0.08);
    background: #fff;
    z-index: 9;
...
.re-select {
    ...
    padding: computed(13) computed(19);
    font-size: $font-common;
}
.count_input {
    box-sizing: border-box;
