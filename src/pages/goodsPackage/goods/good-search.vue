    ...
    <div class="search-box">
        <form action="javascript:return true" class="forms">
            <input
                confirm-type="search"
                v-model="value"
                @confirm="onSearch(value)"
                class="inputs"
                :focus="true"
                :placeholder="holder"
            />
            <p @click="cancel" class="cancels">取消</p>
            <i class="iconfont iconguanbi" @click="delvalue" v-if="value !=''"></i>
        </form>
        <div class="history">
            <div class="history-title" v-if="HistoryList.length !==0">
                <span>历史记录</span>
...
export default {
    ...
    methods: {
        ...
        onSearch(value) {
            ...
            } else if (this.value || this.holder != "搜索商品") {
                this.value = this.value ? this.value : this.holder;
                this.$router.replace(
                    "/pages/goodsPackage/goods/good-list?keywords=" + this.value
                );
            } else {
                Toast("请输入搜索商品~");
                return;
            ...
            if (this.HistoryList.length > 0) {
                // 有数据的话 判断
                console.log(this.value, "valuesss");
                if (this.HistoryList.indexOf(this.value) != -1) {
                    // 有相同的，先删除 再添加
                    this.HistoryList.splice(
                        this.HistoryList.indexOf(this.value),
                        1
                    );
        ...
        // 选择热门搜索
        choiceHot(index) {
            this.value = this.hotList[index].searchWord;
            global.Storage.set("HistoryList", JSON.stringify(this.HistoryList));
            this.onSearch();
        },
        // 获取热门搜索列表
        getHotList(callback) {
            Goods.getHotGoods().then(res => {
                console.log("hot", res);
                res.forEach(item => {
