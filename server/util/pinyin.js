 ...
 * @Date: 2018-08-15 11:31:05
 * @LastEditors: yongtian.hong
 * @LastEditTime: 2018-08-15 11:38:14
 * @Description: 获取汉字第一个字首字母并按首字母进行数据格式化
 * @getFullSpell 获取汉字全拼 
 * @getFirstLetter 获取首字母
 * @sortByFirstLetter 根据首字母分类排序
 */
...
const pinyin = {
    ...
    getFirstLetter: function (str) {
        var firstLetter = this.getFullSpell(str).charAt(0);
        return firstLetter;
    },
    sortByFirstLetter: function (arr, label) {
        if (!arr instanceof Array) {
            return console.log('传入的数据必须是对象数组')
        }
        let key = label || "name"
        let newData = {}; //存新的数据
        let objData = {}
        let letters = []; //存去重后所有数据首字母
        arr.forEach(item => {
            var letter = this.getFirstLetter(item[key]);
            var letter = pinyin.getFirstLetter(item[key]);
            // 首字母作为键
