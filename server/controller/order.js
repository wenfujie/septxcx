...
class orderController {
    ...
    static async getOrderListOnly(ctx, next) {
        let res = await order.getOrderList(ctx, ctx.params);
        ctx.body = res.data;
    }
    ...
    // 确认收货
    static async confirmReceived(ctx, next) {
        let res = await order.confirmReceived(ctx, ctx.params);
        if (res === 1) res = 200;
        ctx.body = {
            status: res
        };
    ...
    // 执行会员升级
    static async packLevelUp(ctx, next) {
    ...
    static async lockStock(ctx, next) {
        let res = await order.lockStock(ctx, ctx.params);
        ctx.body = res;
    }
    ...
    static async saveOrder(ctx, next) {
        let res = await order.saveOrder(ctx, ctx.params);
        ctx.body = res;
    }
    ...
    static async saleCtmmeasureAggregate(ctx, next) {
        let res = await order.saleCtmmeasureAggregate(ctx, ctx.params);
        ctx.body = res;
    }
    ...
    // 提交订单评价
    static async saveOrderComment(ctx, next) {
        let res = await order.saveOrderComment(ctx, ctx.params);
        ctx.body = res;
    }
    ...
    static async getOrderPreferCal(ctx, params) {
        let res = await order.getOrderPreferCal(ctx, ctx.params);
        ctx.body = {
            status: 200,
            data: res.data
        };
